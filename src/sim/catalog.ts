...
const GROWABLES: BuildingDef[] = [
  ...
  g('res_walkup', R1, 4, 75, [M, H]),
  g('res_tenement', R1, 5, 210, [M, H]),
  g('res_projects', R1, 6, 650, [H]),
  g('res_highrise_slab', R1, 7, 1350, [H]),
  // ---------------- R$$ (middle class)
  g('res_cottage', R2, 1, 8, [L]),
  g('res_suburban', R2, 2, 14, [L, M]),
  g('res_ranch', R2, 3, 20, [L]),
  g('res_rowhouses', R2, 3, 50, [M, H]),
  g('res_apartment', R2, 4, 190, [M, H]),
  g('res_courtyard', R2, 5, 480, [M, H]),
  g('res_tower', R2, 6, 880, [H]),
  g('res_tower', R2, 7, 1200, [H]),
  g('res_twin_towers', R2, 8, 2400, [H]),
  // ---------------- R$$$ (wealthy)
  g('res_villa', R3, 1, 6, [L, M]),
  g('res_villa', R3, 2, 8, [L]),
  g('res_mansion', R3, 3, 12, [L]),
  g('res_condo', R3, 4, 150, [M, H]),
  g('res_condo', R3, 5, 210, [M, H]),
  g('res_luxury_tower', R3, 6, 1050, [H]),
  g('res_luxury_tower', R3, 7, 1500, [H]),
  ...
  g('com_supermarket', CS1, 4, 90, [M, H], { noise: 0.15, radius: 3 }),
  g('com_hotel', CS1, 5, 160, [M, H], { noise: 0.1, radius: 2 }),
  // ---------------- CS$$
  g('com_restaurant', CS2, 1, 15, [L, M], { noise: 0.05, radius: 1 }),
  g('com_strip_mall', CS2, 2, 60, [L, M], { noise: 0.15, radius: 2 }),
  g('com_supermarket', CS2, 3, 100, [M, H], { noise: 0.15, radius: 3 }),
  g('com_hotel', CS2, 4, 180, [M, H], { noise: 0.1, radius: 2 }),
  g('com_mall', CS2, 6, 560, [H], { noise: 0.25, radius: 4 }),
  g('com_hotel_tower', CS2, 7, 800, [H], { noise: 0.15, radius: 3 }),
  // ---------------- CS$$$
  g('com_boutique', CS3, 1, 12, [L, M, H], { noise: 0.03, radius: 1 }),
  g('com_restaurant', CS3, 2, 20, [L, M], { noise: 0.05, radius: 1 }),
  g('com_department_store', CS3, 4, 230, [M, H], { noise: 0.1, radius: 2 }),
  g('com_hotel_tower', CS3, 7, 960, [H], { noise: 0.15, radius: 3 }),
  // ---------------- CO$$ (offices)
  g('com_office_small', CO2, 3, 200, [M, H]),
  g('com_office_block', CO2, 4, 320, [M, H]),
  g('com_office_tower', CO2, 6, 1120, [H], { noise: 0.1, radius: 2 }),
  g('com_office_tower', CO2, 7, 1440, [H], { noise: 0.1, radius: 2 }),
  // ---------------- CO$$$
  g('com_office_small', CO3, 3, 180, [M, H]),
  g('com_office_block', CO3, 5, 360, [M, H]),
  g('com_office_tower', CO3, 6, 1280, [H], { noise: 0.1, radius: 2 }),
  g('com_skyscraper', CO3, 7, 3200, [H], { noise: 0.15, radius: 3 }),
  ...
  g('ind_refinery', ID, 4, 270, [M], { air: 0.8, water: 0.5, noise: 0.5, radius: 11 }),
  // ---------------- I-M (manufacturing)
  ...
  g('ind_assembly_plant', IM, 4, 320, [M, H], { air: 0.2, water: 0.1, noise: 0.35, radius: 6 }),
  ...
  g('ind_tech_campus', IHT, 4, 420, [H], { noise: 0.05, radius: 2 }),
  g('ind_tech_campus', IHT, 5, 580, [H], { noise: 0.05, radius: 2 }),
];
