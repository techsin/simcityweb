...
const GROWABLES: BuildingDef[] = [
  ...
  g('com_supermarket', CS1, 4, 140, [M, H], { noise: 0.15, radius: 3 }),
  g('com_hotel', CS1, 5, 250, [M, H], { noise: 0.1, radius: 2 }),
  // ---------------- CS$$
  g('com_restaurant', CS2, 1, 15, [L, M], { noise: 0.05, radius: 1 }),
  g('com_strip_mall', CS2, 2, 60, [L, M], { noise: 0.15, radius: 2 }),
  g('com_supermarket', CS2, 3, 150, [M, H], { noise: 0.15, radius: 3 }),
  g('com_hotel', CS2, 4, 280, [M, H], { noise: 0.1, radius: 2 }),
  g('com_mall', CS2, 6, 1100, [H], { noise: 0.25, radius: 4 }),
  g('com_hotel_tower', CS2, 7, 1600, [H], { noise: 0.15, radius: 3 }),
  // ---------------- CS$$$
  g('com_boutique', CS3, 1, 12, [L, M, H], { noise: 0.03, radius: 1 }),
  g('com_restaurant', CS3, 2, 20, [L, M], { noise: 0.05, radius: 1 }),
  g('com_department_store', CS3, 4, 350, [M, H], { noise: 0.1, radius: 2 }),
  g('com_hotel_tower', CS3, 7, 2000, [H], { noise: 0.15, radius: 3 }),
  // ---------------- CO$$ (offices)
  g('com_office_small', CO2, 3, 240, [M, H]),
  g('com_office_block', CO2, 4, 520, [M, H]),
  g('com_office_tower', CO2, 6, 2400, [H], { noise: 0.1, radius: 2 }),
  g('com_office_tower', CO2, 7, 3200, [H], { noise: 0.1, radius: 2 }),
  // ---------------- CO$$$
  g('com_office_small', CO3, 3, 220, [M, H]),
  g('com_office_block', CO3, 5, 600, [M, H]),
  g('com_office_tower', CO3, 6, 2800, [H], { noise: 0.1, radius: 2 }),
  g('com_skyscraper', CO3, 7, 6500, [H], { noise: 0.15, radius: 3 }),
  ...
  g('ind_smokestack_factory', ID, 3, 220, [M], { air: 0.6, water: 0.35, noise: 0.45, radius: 8 }),
  g('ind_refinery', ID, 4, 450, [M], { air: 0.8, water: 0.5, noise: 0.5, radius: 11 }),
  // ---------------- I-M (manufacturing)
  g('ind_workshop', IM, 1, 25, [M], { air: 0.1, water: 0.05, noise: 0.25, radius: 3 }),
  g('ind_depot', IM, 2, 70, [M, H], { air: 0.08, noise: 0.35, radius: 4 }),
  g('ind_warehouse', IM, 2, 100, [M, H], { air: 0.06, noise: 0.3, radius: 4 }),
  g('ind_warehouse', IM, 3, 140, [M, H], { air: 0.08, noise: 0.3, radius: 4 }),
  g('ind_assembly_plant', IM, 4, 520, [M, H], { air: 0.2, water: 0.1, noise: 0.35, radius: 6 }),
  // ---------------- I-HT (clean high tech, needs EQ)
  g('ind_lab', IHT, 2, 160, [H], { noise: 0.05, radius: 2 }),
  g('ind_datacenter', IHT, 3, 120, [H], { noise: 0.15, radius: 2, powerMul: 12 }),
  g('ind_tech_campus', IHT, 4, 700, [H], { noise: 0.05, radius: 2 }),
  g('ind_tech_campus', IHT, 5, 950, [H], { noise: 0.05, radius: 2 }),
];
...
const PLOPPABLES: BuildingDef[] = [
  ...
  // ================================================================ POLICE
  p({ id: 'civ_police_kiosk', name: 'Police Kiosk', category: 'police', service: 'police', cost: 400, upkeep: 80, jobs: 6, powerUse: 0.05, waterUse: 2,
    coverage: { kind: 'police', radius: 10, strength: 0.6 }, description: 'Small neighborhood police post.' }),
  p({ id: 'civ_police_station', name: 'Police Station', category: 'police', service: 'police', cost: 1500, upkeep: 300, jobs: 30, powerUse: 0.2, waterUse: 8,
    coverage: { kind: 'police', radius: 22, strength: 0.85 }, description: 'Standard police station.' }),
  p({ id: 'civ_police_hq', name: 'Police Headquarters', category: 'police', service: 'police', cost: 8000, upkeep: 1100, jobs: 120, powerUse: 0.8, waterUse: 30,
    coverage: { kind: 'police', radius: 38, strength: 1.0 }, requires: 'police_hq', description: 'Large, well equipped police HQ.' }),
  ...
  // ================================================================ FIRE
  p({ id: 'civ_fire_station', name: 'Fire Station', category: 'fire', service: 'fire', cost: 1400, upkeep: 280, jobs: 25, powerUse: 0.15, waterUse: 20,
    coverage: { kind: 'fire', radius: 20, strength: 0.85 }, description: 'Standard fire station.' }),
  p({ id: 'civ_fire_hq', name: 'Fire Headquarters', category: 'fire', service: 'fire', cost: 7000, upkeep: 950, jobs: 90, powerUse: 0.5, waterUse: 60,
    coverage: { kind: 'fire', radius: 34, strength: 1.0 }, requires: 'fire_hq', description: 'Large fire HQ with training tower.' }),

  // ================================================================ HEALTH
  p({ id: 'civ_clinic', name: 'Medical Clinic', category: 'health', service: 'health', cost: 1800, upkeep: 320, jobs: 30, powerUse: 0.2, waterUse: 15,
    coverage: { kind: 'health', radius: 14, strength: 0.7, capacity: 6000 }, description: 'Neighborhood clinic (6,000 residents).' }),
  p({ id: 'civ_hospital', name: 'Hospital', category: 'health', service: 'health', cost: 9000, upkeep: 1400, jobs: 250, powerUse: 1.5, waterUse: 120,
    coverage: { kind: 'health', radius: 32, strength: 1.0, capacity: 35000 }, description: 'Full hospital (35,000 residents).' }),
  ...
  // ================================================================ EDUCATION
  p({ id: 'civ_elementary_school', name: 'Elementary School', category: 'education', service: 'education', cost: 2000, upkeep: 380, jobs: 40, powerUse: 0.2, waterUse: 20,
    coverage: { kind: 'education', radius: 18, strength: 0.7, capacity: 10000 }, description: 'Schools the kids of 10,000 residents.' }),
  p({ id: 'civ_high_school', name: 'High School', category: 'education', service: 'education', cost: 6000, upkeep: 850, jobs: 90, powerUse: 0.5, waterUse: 50,
    coverage: { kind: 'education', radius: 28, strength: 0.9, capacity: 25000 }, description: 'Serves 25,000 residents. Raises EQ.' }),
  p({ id: 'civ_college', name: 'University', category: 'education', service: 'education', cost: 25000, upkeep: 2600, jobs: 400, powerUse: 2, waterUse: 200,
  ...
  p({ id: 'civ_library', name: 'Public Library', category: 'education', service: 'education', cost: 2500, upkeep: 300, jobs: 20, powerUse: 0.2, waterUse: 5,
  ...
  p({ id: 'rw_military_base', name: 'Military Base', category: 'reward', cost: 0, upkeep: 0, income: 2500, jobs: 400, powerUse: 3, waterUse: 150,
  ...
  p({ id: 'rw_missile_range', name: 'Missile Test Range', category: 'reward', cost: 0, upkeep: 0, income: 4000, jobs: 150, powerUse: 2, waterUse: 40,
  ...
  p({ id: 'rw_toxic_dump', name: 'Toxic Waste Dump', category: 'reward', cost: 0, upkeep: 0, income: 6000, jobs: 60, powerUse: 0.5,
