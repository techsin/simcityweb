 ...
 *   coverage     radius in cells (16 m), strength 0..1 at the source. Service tiers (WP2 catchments, infra/catchments.ts):
 *                `tier` + `metric` (walk over streets / drive over roads / euclid disk) + `capacity` in TIER UNITS —
 *                pupils (elementary = kids 0-11, high = teens), students (college / library), patient-equivalents
 *                (clinic / hospital; seniors need 3.5x), visitors (play / green parks). Capacity is shared among
 *                everyone in the catchment (crowding lowers coverage). Police / fire are capacity-free (radius x
 *                strength). Without `tier`, capacity = legacy "residents served" (converted by the reference mix).
 *   stigma / prestige / campus  NIMBY / YIMBY splats {amount 0..1 at the source, radius cells} (infra/nimby.ts).
...
const PLOPPABLES: BuildingDef[] = [
  ...
  p({ id: 'util_wind_turbine', name: 'Wind Turbine', category: 'power', service: 'utilities', cost: 800, upkeep: 16, jobs: 1, powerOut: 5,
    pollution: { noise: 0.25, radius: 3 }, stigma: { amount: 0.05, radius: 3 },
  ...
  p({ id: 'util_gas_plant', name: 'Natural Gas Power Plant', category: 'power', service: 'utilities', cost: 22000, upkeep: 650, jobs: 90, powerOut: 250, waterUse: 400,
    pollution: { air: 0.45, noise: 0.3, radius: 9, garbage: 10 }, landValue: { amount: -0.12, radius: 7 }, stigma: { amount: 0.25, radius: 8 },
    description: '250 MW. Cleaner than coal, moderate cost. Fuel costs scale with output. Neighbours dislike it a little.' }),
  p({ id: 'util_coal_plant', name: 'Coal Power Plant', category: 'power', service: 'utilities', cost: 25000, upkeep: 700, jobs: 180, powerOut: 400, waterUse: 800,
    pollution: { air: 1.0, water: 0.2, noise: 0.4, radius: 14, garbage: 40 }, landValue: { amount: -0.25, radius: 10 }, stigma: { amount: 0.55, radius: 12 },
    description: '400 MW of cheap, dirty power. Heavy air pollution — keep it downwind of homes. Nobody wants to live next to it (strong stigma within 12 tiles).' }),
  p({ id: 'util_oil_plant', name: 'Oil Power Plant', category: 'power', service: 'utilities', cost: 30000, upkeep: 800, jobs: 150, powerOut: 350, waterUse: 600,
    pollution: { air: 0.8, water: 0.25, noise: 0.35, radius: 12, garbage: 25 }, landValue: { amount: -0.2, radius: 9 }, stigma: { amount: 0.45, radius: 10 },
    description: '350 MW. Pollutes less than coal but costs more to run. Unwanted neighbour (stigma within 10 tiles).' }),
  p({ id: 'util_hydro_dam', name: 'Hydroelectric Station', category: 'power', service: 'utilities', cost: 12000, upkeep: 220, jobs: 25, powerOut: 80, placement: 'shore',
    pollution: { water: 0.05, noise: 0.1, radius: 3 }, stigma: { amount: 0.05, radius: 4 },
  ...
  p({ id: 'util_solar_farm', name: 'Solar Farm', category: 'power', service: 'utilities', cost: 20000, upkeep: 200, jobs: 12, powerOut: 60, requires: 'solar_power',
    stigma: { amount: 0.03, radius: 2 }, description: '60 MW of silent, clean power. Expensive per MW.' }),
  p({ id: 'util_nuclear_plant', name: 'Nuclear Power Plant', category: 'power', service: 'utilities', cost: 120000, upkeep: 2800, jobs: 400, powerOut: 1600, waterUse: 3000,
    ...
    stigma: { amount: 0.5, radius: 14 }, description: '1600 MW of clean power for a metropolis. Very expensive; residents dislike living next to it (stigma within 14 tiles).' }),
  ...
  p({ id: 'util_water_treatment', name: 'Water Treatment Plant', category: 'water', service: 'utilities', cost: 15000, upkeep: 450, jobs: 60, waterOut: 50000, powerUse: 8,
    pollution: { water: -0.5, radius: 16 }, requires: 'water_treatment', stigma: { amount: 0.15, radius: 5 },
    description: '50,000 kL/day of clean water and it cleans nearby water pollution. Neighbours mind the smell a little.' }),
  p({ id: 'util_desalination', name: 'Desalination Plant', category: 'water', service: 'utilities', cost: 32000, upkeep: 900, jobs: 80, waterOut: 80000, powerUse: 25,
    placement: 'shore', requires: 'desalination', pollution: { noise: 0.15, radius: 3 }, stigma: { amount: 0.1, radius: 4 },
  ...
  p({ id: 'util_landfill_tile', name: 'Landfill', category: 'garbage', service: 'utilities', cost: 15, upkeep: 1, garbageCapacity: 300, hidden: true,
    ...
    stigma: { amount: 0.35, radius: 6 }, // per 2x2 block of landfill cells, scaled by use (infra/nimby.ts)
  ...
  p({ id: 'util_recycling_center', name: 'Recycling Center', category: 'garbage', service: 'utilities', cost: 9000, upkeep: 260, jobs: 60, garbageCapacity: 4000, powerUse: 2,
    pollution: { noise: 0.2, radius: 4 }, requires: 'recycling_center', stigma: { amount: 0.15, radius: 5 },
    description: 'Processes 4,000 t/month of garbage cleanly. Mildly unwanted next door.' }),
  p({ id: 'util_incinerator', name: 'Waste-to-Energy Incinerator', category: 'garbage', service: 'utilities', cost: 26000, upkeep: 700, jobs: 70, garbageCapacity: 12000, powerOut: 60,
    pollution: { air: 0.6, noise: 0.25, radius: 10 }, landValue: { amount: -0.15, radius: 8 }, requires: 'incinerator', stigma: { amount: 0.4, radius: 10 },
    description: 'Burns 12,000 t/month of garbage and generates 60 MW. Pollutes the air; nobody wants to live next to it.' }),
  ...
  p({ id: 'civ_police_kiosk', name: 'Police Kiosk', category: 'police', service: 'police', cost: 400, upkeep: 50, jobs: 6, powerUse: 0.05, waterUse: 2,
    ...
    description: 'Small neighbourhood police post: patrols reach about 12 tiles by road. Needs power.' }),
  p({ id: 'civ_police_station', name: 'Police Station', category: 'police', service: 'police', cost: 1500, upkeep: 170, jobs: 30, powerUse: 0.2, waterUse: 8,
    ...
    description: 'Standard police station: patrols reach about 26 tiles by road (avenues and highways reach farther, streets less). Needs power.' }),
  p({ id: 'civ_police_hq', name: 'Police Headquarters', category: 'police', service: 'police', cost: 8000, upkeep: 900, jobs: 120, powerUse: 0.8, waterUse: 30,
    ...
    description: 'Large, well equipped police HQ: full-strength patrols about 42 tiles by road. Needs power.' }),
  p({ id: 'civ_jail', name: 'Prison', category: 'police', service: 'police', cost: 12000, upkeep: 900, jobs: 150, powerUse: 1, waterUse: 120,
    coverage: { kind: 'police', radius: 64, strength: 0.15, capacity: 80000, tier: 'police', metric: 'drive' }, landValue: { amount: -0.3, radius: 8 }, requires: 'jail',
  ...
  p({ id: 'civ_fire_station', name: 'Fire Station', category: 'fire', service: 'fire', cost: 1400, upkeep: 160, jobs: 25, powerUse: 0.15, waterUse: 20,
    coverage: { kind: 'fire', radius: 24, strength: 0.85, tier: 'fire', metric: 'drive' },
    description: 'Standard fire station: fire prevention and response about 24 tiles by road. Needs power.' }),
  p({ id: 'civ_fire_hq', name: 'Fire Headquarters', category: 'fire', service: 'fire', cost: 7000, upkeep: 800, jobs: 90, powerUse: 0.5, waterUse: 60,
    coverage: { kind: 'fire', radius: 38, strength: 1.0, tier: 'fire', metric: 'drive' }, requires: 'fire_hq',
    description: 'Large fire HQ with training tower: full-strength fire protection about 38 tiles by road. Needs power.' }),
  ...
  p({ id: 'civ_elementary_school', name: 'Elementary School', category: 'education', service: 'education', cost: 2000, upkeep: 240, jobs: 40, powerUse: 0.2, waterUse: 20,
    coverage: { kind: 'education', radius: 20, strength: 1.0, capacity: 1500, tier: 'elementary', metric: 'walk' },
    description: 'Seats 1,500 pupils (ages 0-11) within a ~20-tile walk; highways and unbridged water block the walk. A crowded school teaches everyone less; needs power.' }),
  p({ id: 'civ_high_school', name: 'High School', category: 'education', service: 'education', cost: 6000, upkeep: 600, jobs: 90, powerUse: 0.5, waterUse: 50,
    coverage: { kind: 'education', radius: 32, strength: 1.0, capacity: 2100, tier: 'high', metric: 'drive' },
    description: 'Seats 2,100 teens (ages 12-17) within ~32 tiles by school bus. Raises EQ over the years; needs power.' }),
  p({ id: 'civ_college', name: 'University', category: 'education', service: 'education', cost: 25000, upkeep: 2200, jobs: 400, powerUse: 2, waterUse: 200,
    coverage: { kind: 'education', radius: 60, strength: 1.0, capacity: 6000, tier: 'college', metric: 'drive' }, landValue: { amount: 0.15, radius: 12 }, requires: 'college',
    campus: { amount: 1.0, radius: 18 },
    description: 'University: 6,000 students (young adults and some adults) within ~60 tiles by road. Big EQ boost; offices and high-tech industry love a campus nearby. Needs power.' }),
  p({ id: 'civ_library', name: 'Public Library', category: 'education', service: 'education', cost: 2500, upkeep: 180, jobs: 20, powerUse: 0.2, waterUse: 5,
    coverage: { kind: 'education', radius: 24, strength: 0.35, capacity: 3000, tier: 'library', metric: 'walk' }, landValue: { amount: 0.05, radius: 6 },
    campus: { amount: 0.2, radius: 6 },
    description: 'Lifelong learning: partial college-level education for 3,000 within a ~24-tile walk; mild EQ boost. Needs power.' }),
  p({ id: 'civ_museum', name: 'Museum', category: 'education', service: 'education', cost: 8000, upkeep: 650, jobs: 30, powerUse: 0.4, waterUse: 10,
    coverage: { kind: 'education', radius: 40, strength: 0.25, capacity: 8000, tier: 'library', metric: 'drive' }, landValue: { amount: 0.15, radius: 10 }, requires: 'museum',
    prestige: { amount: 0.3, radius: 12 },
    description: 'Culture! Partial college-level learning for 8,000 visitors within ~40 tiles by road; raises land value and wealthy residents like living near it. Needs power.' }),
  ...
  p({ id: 'civ_mayor_house', name: "Mayor's House", category: 'civic', cost: 2000, upkeep: 50, jobs: 4, landValue: { amount: 0.2, radius: 8 }, requires: 'mayor_house', unique: true,
    prestige: { amount: 0.3, radius: 8 }, description: 'Reward: a stately home for the mayor. Raises land value and prestige around it.' }),
  ...
  p({ id: 'civ_cemetery', name: 'Cemetery', category: 'civic', cost: 1200, upkeep: 60, jobs: 4, landValue: { amount: 0.03, radius: 4 }, requires: 'cemetery',
    coverage: { kind: 'park', radius: 4, strength: 0.4, capacity: 1500, tier: 'green', metric: 'walk' }, stigma: { amount: 0.06, radius: 3 },
    description: 'Quiet green space for 1,500 within a 4-tile walk, valued by seniors; a few next-door neighbours find it gloomy.' }),
  p({ id: 'civ_courthouse', name: 'Courthouse', category: 'civic', service: 'police', cost: 6000, upkeep: 400, jobs: 60, powerUse: 0.3, waterUse: 10,
    coverage: { kind: 'police', radius: 64, strength: 0.1, tier: 'police', metric: 'drive' }, landValue: { amount: 0.1, radius: 8 }, requires: 'courthouse', unique: true,
    prestige: { amount: 0.2, radius: 8 },
  ...
  p({ id: 'civ_city_hall', name: 'City Hall', category: 'civic', cost: 20000, upkeep: 900, jobs: 200, powerUse: 1, waterUse: 40,
    landValue: { amount: 0.2, radius: 14 }, requires: 'city_hall', unique: true, prestige: { amount: 0.35, radius: 14 },
    description: 'Reward: the seat of government. Attracts offices and residents; a prestigious address for its neighbours.' }),
  p({ id: 'civ_convention_center', name: 'Convention Center', category: 'civic', cost: 30000, upkeep: 1200, income: 500, jobs: 250, powerUse: 2, waterUse: 60,
    landValue: { amount: 0.1, radius: 10 }, pollution: { noise: 0.2, radius: 4 }, requires: 'convention_center', unique: true, prestige: { amount: 0.2, radius: 10 },
    description: 'Reward: brings business visitors. Big boost to commercial demand cap.' }),

  // ================================================================ PARKS & RECREATION
  p({ id: 'park_small', name: 'Small Park', category: 'park', service: 'parks', cost: 150, upkeep: 12, coverage: { kind: 'park', radius: 6, strength: 0.7, capacity: 2500, tier: 'green', metric: 'walk' },
    landValue: { amount: 0.12, radius: 5 }, description: 'Lawn, trees and benches: green space for 2,500 within a 6-tile walk. Raises the residential demand cap.' }),
  p({ id: 'park_playground', name: 'Playground', category: 'park', service: 'parks', cost: 250, upkeep: 10, coverage: { kind: 'park', radius: 6, strength: 0.9, capacity: 700, tier: 'play', metric: 'walk' },
    landValue: { amount: 0.08, radius: 4 }, description: 'Play space for 700 kids and teens within a 6-tile walk. Families love it.' }),
  p({ id: 'park_basketball', name: 'Basketball Court', category: 'park', service: 'parks', cost: 300, upkeep: 12, coverage: { kind: 'park', radius: 6, strength: 0.8, capacity: 500, tier: 'play', metric: 'walk' },
    landValue: { amount: 0.06, radius: 4 }, description: 'Neighbourhood court: play space for 500 kids and teens within a 6-tile walk.' }),
  p({ id: 'park_tennis', name: 'Tennis Courts', category: 'park', service: 'parks', cost: 450, upkeep: 15, coverage: { kind: 'park', radius: 6, strength: 0.6, capacity: 600, tier: 'play', metric: 'walk' },
    landValue: { amount: 0.1, radius: 5 }, description: 'Play space for 600 within a 6-tile walk. Popular with the well-off.' }),
  p({ id: 'park_plaza', name: 'Plaza', category: 'park', service: 'parks', cost: 500, upkeep: 25, coverage: { kind: 'park', radius: 7, strength: 0.6, capacity: 4000, tier: 'green', metric: 'walk' },
    landValue: { amount: 0.15, radius: 6 }, description: 'Paved plaza with fountain: green space for 4,000 within a 7-tile walk. Great downtown.' }),
  p({ id: 'park_garden', name: 'Formal Garden', category: 'park', service: 'parks', cost: 1200, upkeep: 45, coverage: { kind: 'park', radius: 8, strength: 0.9, capacity: 5000, tier: 'green', metric: 'walk' },
    landValue: { amount: 0.2, radius: 8 }, prestige: { amount: 0.2, radius: 8 },
    description: 'Hedges and flower beds: green space for 5,000 within an 8-tile walk. Wealthy residents love it (prestige).' }),
  p({ id: 'park_soccer', name: 'Soccer Field', category: 'park', service: 'parks', cost: 1200, upkeep: 40, coverage: { kind: 'park', radius: 10, strength: 0.9, capacity: 1500, tier: 'play', metric: 'walk' },
    landValue: { amount: 0.08, radius: 7 }, pollution: { noise: 0.1, radius: 3 }, description: 'Field with stands and lights: play space for 1,500 kids and teens within a 10-tile walk.' }),
  p({ id: 'park_baseball', name: 'Baseball Diamond', category: 'park', service: 'parks', cost: 1800, upkeep: 55, coverage: { kind: 'park', radius: 12, strength: 0.9, capacity: 2000, tier: 'play', metric: 'walk' },
    landValue: { amount: 0.08, radius: 8 }, pollution: { noise: 0.1, radius: 3 }, description: 'Diamond with bleachers and lights: play space for 2,000 within a 12-tile walk.' }),
  p({ id: 'park_marina', name: 'Marina', category: 'park', service: 'parks', cost: 2500, upkeep: 60, placement: 'shore', coverage: { kind: 'park', radius: 10, strength: 0.7, capacity: 6000, tier: 'green', metric: 'euclid' },
    landValue: { amount: 0.25, radius: 10 }, prestige: { amount: 0.35, radius: 10 },
    description: 'Docks and boats: green space for 6,000 within 10 tiles. Prestigious waterfront luxury (front faces water).' }),
  p({ id: 'park_large', name: 'Large Park', category: 'park', service: 'parks', cost: 3000, upkeep: 90, coverage: { kind: 'park', radius: 14, strength: 1.0, capacity: 20000, tier: 'green', metric: 'walk' },
    landValue: { amount: 0.25, radius: 12 }, description: 'Pond, paths and meadows: green space for 20,000 within a 14-tile walk. A big boost to residential demand cap.' }),
  p({ id: 'park_zoo', name: 'Zoo', category: 'park', service: 'parks', cost: 20000, upkeep: 900, income: 300, jobs: 80, powerUse: 0.5, waterUse: 100,
    coverage: { kind: 'park', radius: 30, strength: 0.9, capacity: 60000, tier: 'green', metric: 'drive' }, landValue: { amount: 0.15, radius: 16 }, requires: 'zoo',
    description: 'Lions and tigers and bears: green space for 60,000 within ~30 tiles by road. Huge residential cap boost, attracts visitors. Needs power.' }),
  p({ id: 'park_golf', name: 'Country Club', category: 'park', service: 'parks', cost: 15000, upkeep: 400, jobs: 40, waterUse: 400,
    coverage: { kind: 'park', radius: 16, strength: 0.8, capacity: 4000, tier: 'green', metric: 'euclid' }, landValue: { amount: 0.35, radius: 16 }, requires: 'country_club', unique: true,
    prestige: { amount: 0.5, radius: 16 },
    description: 'Reward: golf for the elite (green space for 4,000 within 16 tiles). Strong prestige: a magnet for R$$$ residents.' }),
  p({ id: 'park_stadium', name: 'Major League Stadium', category: 'park', service: 'parks', cost: 60000, upkeep: 1800, income: 1500, jobs: 300, powerUse: 3, waterUse: 150,
    coverage: { kind: 'park', radius: 24, strength: 0.6, capacity: 80000, tier: 'play', metric: 'drive' }, landValue: { amount: 0.1, radius: 12 }, pollution: { noise: 0.5, radius: 10 }, requires: 'stadium', unique: true,
    description: 'Reward: big-league sports — play space for 80,000 within ~24 tiles by road. Raises residential and commercial caps. Loud. Needs power.' }),
  p({ id: 'park_amusement', name: 'Amusement Park', category: 'park', service: 'parks', cost: 45000, upkeep: 1500, income: 1200, jobs: 250, powerUse: 3, waterUse: 150,
    coverage: { kind: 'park', radius: 24, strength: 0.7, capacity: 80000, tier: 'play', metric: 'drive' }, landValue: { amount: 0.1, radius: 12 }, pollution: { noise: 0.3, radius: 8 }, requires: 'amusement_park', unique: true,
    description: 'Roller coasters and a ferris wheel: play space for 80,000 within ~24 tiles by road. Tourists flock to it. Needs power.' }),

  // ================================================================ LANDMARKS (unlocked by population)
  p({ id: 'lm_lighthouse', name: 'Lighthouse', category: 'landmark', cost: 3000, upkeep: 20, placement: 'shore', landValue: { amount: 0.15, radius: 8 }, prestige: { amount: 0.25, radius: 8 }, requires: 'lm_lighthouse', unique: true,
    description: 'Landmark (3,000 pop). Must stand on the shore.' }),
  p({ id: 'lm_clock_tower', name: 'Clock Tower', category: 'landmark', cost: 6000, upkeep: 40, landValue: { amount: 0.2, radius: 10 }, prestige: { amount: 0.3, radius: 10 }, requires: 'lm_clock_tower', unique: true,
    description: 'Landmark (5,000 pop).' }),
  p({ id: 'lm_obelisk', name: 'Obelisk', category: 'landmark', cost: 10000, upkeep: 50, landValue: { amount: 0.2, radius: 12 }, prestige: { amount: 0.3, radius: 12 }, requires: 'lm_obelisk', unique: true,
    description: 'Landmark (10,000 pop).' }),
  p({ id: 'lm_arch', name: 'Triumphal Arch', category: 'landmark', cost: 15000, upkeep: 60, landValue: { amount: 0.25, radius: 12 }, prestige: { amount: 0.35, radius: 12 }, requires: 'lm_arch', unique: true,
  ...
  p({ id: 'lm_observatory', name: 'Observatory', category: 'landmark', cost: 20000, upkeep: 150, jobs: 20, powerUse: 0.3,
    coverage: { kind: 'education', radius: 30, strength: 0.2, capacity: 5000, tier: 'library', metric: 'drive' }, landValue: { amount: 0.2, radius: 12 }, prestige: { amount: 0.35, radius: 12 },
    campus: { amount: 0.3, radius: 10 }, requires: 'lm_observatory', unique: true,
    description: 'Landmark (25,000 pop). Mild college-level education for 5,000 within ~30 tiles by road; a small campus draw for high-tech. Needs power.' }),
  p({ id: 'lm_cathedral', name: 'Cathedral', category: 'landmark', cost: 30000, upkeep: 200, jobs: 10, landValue: { amount: 0.3, radius: 14 }, prestige: { amount: 0.45, radius: 14 }, requires: 'lm_cathedral', unique: true,
    description: 'Landmark (35,000 pop).' }),
  p({ id: 'lm_castle', name: 'Castle', category: 'landmark', cost: 40000, upkeep: 250, jobs: 20, landValue: { amount: 0.3, radius: 16 }, prestige: { amount: 0.45, radius: 16 }, requires: 'lm_castle', unique: true,
    description: 'Landmark (50,000 pop). Tourists love it.' }),
  p({ id: 'lm_pyramid', name: 'Glass Pyramid', category: 'landmark', cost: 50000, upkeep: 300, jobs: 20, landValue: { amount: 0.3, radius: 14 }, prestige: { amount: 0.45, radius: 14 }, requires: 'lm_pyramid', unique: true,
  ...
  p({ id: 'lm_ferris_wheel', name: 'Giant Observation Wheel', category: 'landmark', cost: 45000, upkeep: 350, income: 400, jobs: 40, powerUse: 1,
    landValue: { amount: 0.25, radius: 14 }, prestige: { amount: 0.35, radius: 14 }, requires: 'lm_ferris_wheel', unique: true, description: 'Landmark (90,000 pop). Earns ticket income.' }),
  p({ id: 'lm_opera_house', name: 'Opera House', category: 'landmark', cost: 80000, upkeep: 600, jobs: 80, powerUse: 1,
    landValue: { amount: 0.4, radius: 18 }, prestige: { amount: 0.6, radius: 18 }, requires: 'lm_opera_house', unique: true, description: 'Landmark (150,000 pop). Spectacular at the waterfront.' }),
  p({ id: 'lm_spire_tower', name: 'Observation Spire', category: 'landmark', cost: 120000, upkeep: 800, income: 800, jobs: 60, powerUse: 2,
    landValue: { amount: 0.35, radius: 20 }, prestige: { amount: 0.5, radius: 20 }, requires: 'lm_spire_tower', unique: true, description: 'Landmark (250,000 pop). Dominates the skyline.' }),
  p({ id: 'lm_twin_spires', name: 'Twin Spires', category: 'landmark', cost: 200000, upkeep: 1200, jobs: 2000, powerUse: 6, waterUse: 100,
    landValue: { amount: 0.45, radius: 22 }, prestige: { amount: 0.6, radius: 22 }, requires: 'lm_twin_spires', unique: true, description: 'Landmark (500,000 pop). The ultimate skyline statement.' }),
  ...
  p({ id: 'rw_military_base', name: 'Military Base', category: 'reward', cost: 0, upkeep: 0, income: 1500, jobs: 400, powerUse: 3, waterUse: 150,
    landValue: { amount: -0.2, radius: 14 }, pollution: { noise: 0.5, air: 0.1, radius: 12 }, requires: 'military_base', unique: true, stigma: { amount: 0.35, radius: 12 },
  ...
  p({ id: 'rw_missile_range', name: 'Missile Test Range', category: 'reward', cost: 0, upkeep: 0, income: 2500, jobs: 150, powerUse: 2, waterUse: 40,
    landValue: { amount: -0.35, radius: 18 }, pollution: { noise: 0.7, air: 0.15, radius: 16 }, requires: 'missile_range', unique: true, stigma: { amount: 0.45, radius: 16 },
  ...
  p({ id: 'rw_toxic_dump', name: 'Toxic Waste Dump', category: 'reward', cost: 0, upkeep: 0, income: 4000, jobs: 60, powerUse: 0.5,
    landValue: { amount: -0.5, radius: 20 }, pollution: { air: 0.5, water: 0.8, radius: 14 }, requires: 'toxic_dump', unique: true, stigma: { amount: 0.95, radius: 22 },
  ...
  p({ id: 'rw_casino', name: 'Casino Resort', category: 'reward', cost: 25000, upkeep: 400, income: 3000, jobs: 500, powerUse: 3, waterUse: 100,
    landValue: { amount: -0.05, radius: 8 }, pollution: { noise: 0.3, radius: 6 }, requires: 'casino', unique: true, stigma: { amount: 0.15, radius: 6 },
  ...
  p({ id: 'rw_research_center', name: 'Advanced Research Center', category: 'reward', service: 'education', cost: 40000, upkeep: 1500, jobs: 600, powerUse: 5, waterUse: 60,
    coverage: { kind: 'education', radius: 50, strength: 0.35, capacity: 10000, tier: 'college', metric: 'drive' }, landValue: { amount: 0.2, radius: 14 },
    requires: 'research_center', unique: true, campus: { amount: 1.0, radius: 24 },
    description: 'Reward (EQ milestone): partial college places for 10,000 within ~50 tiles by road; a strong campus draw for high-tech and offices. Needs power.' }),
  ...
  p({ id: 'civ_bus_depot', name: 'Bus Depot', category: 'transport', service: 'transit', cost: 4000, upkeep: 350, jobs: 50, powerUse: 0.3, waterUse: 10,
    coverage: { kind: 'transit', radius: 30, strength: 0.25 }, pollution: { noise: 0.2, air: 0.05, radius: 4 }, stigma: { amount: 0.08, radius: 3 },
  ...
  p({ id: 'tr_freight_station', name: 'Freight Rail Station', category: 'transport', service: 'transit', cost: 5000, upkeep: 200, jobs: 60, powerUse: 0.3,
    pollution: { noise: 0.3, air: 0.05, radius: 6 }, landValue: { amount: -0.05, radius: 5 }, stigma: { amount: 0.12, radius: 6 },
