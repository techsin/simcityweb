...
import {
  ...
  FREIGHT_BOOST_MAX, I_BASE, I_SHARE_MAX, I_SHARE_MIN, IA_BASE, IA_PER_RES, ID_SHARE_AT_EQ0, ID_SHARE_EQ_SLOPE, ID_SHARE_MIN,
...
export function demandSystem(rt: EconRuntime): SimSystem {
  ...
  const compute = (st: CityState, first: boolean) => {
    ...
    const sID = clamp(ID_SHARE_AT_EQ0 - EQ * ID_SHARE_EQ_SLOPE, ID_SHARE_MIN, 1);
