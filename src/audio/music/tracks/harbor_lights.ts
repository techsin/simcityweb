 ...
 * theme II (16, Rhodes answers the vibes) -> breakdown (8, drums out, motif in augmentation, build) and vibes solo
 * (8/16) in either order -> theme out (8, Rhodes doubles the tune) -> outro (thins out, iv-V-i, ritardando into a
 * held tonic chord).
 *
 ...
 * factory, routed into the 'sweep:wave' channel; envelope 0 -> 0, source always stopped.
 */
function waveWash(env: MusicEnv, t: number, dur: number, vel: number, pan: number, off: number): void {
  const { inst, ctx } = env;
  if (t >= inst.cutoff || (env.live && t < ctx.currentTime)) return;
  if (inst.mute.has('wave') || (inst.solo.size && !inst.solo.has('wave'))) return;
...
export const track: MusicTrack = {
  ...
  create(env) {
    ...
    inst.bpm = bpm;
    const swing = rng.range(0.55, 0.6);
    ...
    FORM.forEach((s, si) => {
      ...
      for (let i = 0; i < s.bars; i++) {
        ...
        plan.push({
          ...
          comp: [], pads: [], bass: [], vibes: [], answer: [], drums: [], fx: [], mix: [], dyn: 1,
        });
      }
    });
    ...
    const realize = (b0: number, notes: readonly Note[], amt: number, o: { endAlt?: boolean; vel?: number; pedal?: boolean; stretch?: number; shift?: number } = {}): Placed[] => {
      ...
      notes.forEach(([nb, nbeat, ndur, ref], i) => {
        ...
        if (i === notes.length - 1 && o.endAlt) {
          const alts = chordTones(c, m - 5, m + 5).filter((x) => x !== m && [0, 3, 7].includes(pc(x - c.root)));
        ...
        if (amt > 0 && dur >= 2 && rng.chance(0.25 * amt)) {
          const half = dur / 2;
    ...
    /** vibes solo: quotes the call, then phrases of 8ths / swung 16th pairs / triplet arpeggios with breaths */
    const soloLine = (b0: number, bars: number): Placed[] => {
      const out: Placed[] = realize(b0, MOT.filter((n) => n[0] < 2), 0.8, { vel: 0.55 });
      const lo = 65 + T, hi = 88 + T;
      let cur = out.length ? out[out.length - 1].e.midi : 76 + T;
      let dir = -1;
      let pos = 8 + rng.pick([0.5, 1, 1.5]);
      const end = bars * 4 - 2;
      while (pos < end - 1) {
        const x = pos / (bars * 4);
        const stop = Math.min(end - 1, pos + rng.int(4, 8 + Math.round(6 * x)) * 0.5);
        let first = true;
        while (pos < stop) {
          ...
          if (!first && beat % 1 === 0 && pos + 1 <= stop && rng.chance(0.06 + 0.06 * x)) {
            ...
            for (let q = 0; q < 3; q++) {
              out.push({ bar: b0 + bar, e: { beat: beat + q / 3, dur: 0.34, midi: m, vel: humVel(rng, 0.5, 0.04), st: true } });
          ...
          if (isChange(b0 + bar, beat) && rng.chance(0.65)) {
            ...
            if (g.length) cur = snap(cur + dir, g);
          } else cur = stepInPool(cur, dir * (rng.chance(0.78) ? 1 : 2), pool);
          if (cur >= hi - 2) dir = -1;
          ...
          if (r < 0.16 + 0.2 * x && pos + 0.5 <= stop) {
            // swung 16th pair
            out.push({ bar: b0 + bar, e: { beat, dur: 0.3, midi: cur, vel } });
            const n2 = stepInPool(cur, dir, pool);
            out.push({ bar: b0 + bar, e: { beat: beat + 0.25, dur: 0.3, midi: n2, vel: vel - 0.04 } });
            cur = n2;
            pos += 0.5;
          } else {
            const step = r < 0.85 ? 0.5 : 1;
            out.push({ bar: b0 + bar, e: { beat, dur: step * 1.1, midi: cur, vel } });
            pos += step;
          }
          first = false;
        }
        if (pos < end) {
          const bar = Math.floor(pos / 4), beat = pos - bar * 4;
          const c = chordAt(b0 + bar, beat);
          cur = snap(cur, chordTones(c, lo, hi));
          const d = Math.min(end - pos, rng.pick([1, 1.5, 2]));
          out.push({ bar: b0 + bar, e: { beat, dur: d, midi: cur, vel: humVel(rng, 0.52, 0.04) } });
          pos += d;
        }
    ...
    FORM.forEach((s, si) => {
      ...
      if (s.kind === 'A') {
        for (let blk = 0; blk < s.bars / 8; blk++) {
          ...
          const ph = realize(b0 + blk * 8, MOT, amt, { endAlt: inner && rng.chance(0.5) });
          place(ph);
          if (s.name === 'theme out') {
        ...
        for (let i = 1; i < s.bars - 1; i += 2) {
          ...
          const tones = chordTones(c, 79 + T, 91 + T);
      ...
      if (g !== 'none') {
        ...
        if (g !== 'kick') {
          hatP.forEach((v, s) => {
            ...
            d.push({ k: open ? 'ohat' : 'hat', step: s, vel: v * 0.42 * hs * (open ? 0.8 : 1) });
          });
          if (g === 'full' && p.sec.kind === 'solo') for (const s of [3, 11]) if (rng.chance(0.25)) d.push({ k: 'hat', step: s, vel: 0.09 });
        }
        if (shaker && g === 'full') for (const s of [2, 6, 10, 14]) d.push({ k: 'shaker', step: s, vel: humVel(rng, 0.24, 0.03) });
    ...
    plan.forEach((p, bi) => {
      ...
      steps.forEach((s, j) => {
        ...
        if (s === pick) {
          ...
          const r = rng.next();
          m = r < 0.5 ? root : r < 0.72 ? (root + 7 <= 47 ? root + 7 : root - 5) : r < 0.86 && root + 12 <= 50 ? root + 12 : c.tones.includes(10) && root + 10 <= 50 ? root + 10 : root;
        }
    ...
    let pushed = false;
    const vopt = { lo: 52, hi: 71, count: 4, rootless: true };
    plan.forEach((p, bi) => {
      const k = p.sec.kind;
      const i = p.inSec;
      const next = plan[bi + 1];
      ...
      const baseVel = k === 'break' ? 0.34 : k === 'B' ? 0.36 : 0.38;
      const vs = p.slots.map((s) => (prevV = voiceLead(prevV, s.chord, vopt)));
      ...
      if (p.slots.length > 1) {
        ...
        for (const [beat, dur, full] of pat) {
          ...
          if (dur === 0) {
            ...
            const nv = voiceLead(prevV, next.slots[0].chord, vopt);
    ...
    FORM.forEach((s, si) => {
      ...
      if (s.kind === 'intro') {
        ...
        for (let i = 0; i < s.bars; i++) plan[b0 + i].mix.push(['epiano', { lowpass: 950 + 2050 * Math.min(1, i / (s.bars - 2)) }]);
      } else if (s.kind === 'break') {
        ...
        for (let i = 3; i < 7; i++) plan[b0 + i].mix.push(['epiano', { lowpass: 2700 - (i - 3) * 400 }]);
        ...
        for (let q = 1; q <= 24; q++) plan[b0 + 6].mix.push([`bass@${1.5 + q * 0.25}`, { level: 0.43 * Math.pow(0.85, q) }]);
        ...
        plan[b0].mix.push(['epiano', { lowpass: 3000 }], ['hat', { lowpass: 10000 }], ['vibes', { tremoloRate: 4.6 }]);
      }
    });
    ...
    const setup = (t0: number, player: { endTime: number }) => {
      ...
      inst.mix('epiano', { level: 1.3, pan: -0.28, reverb: 0.28, delay: 0.03, tremolo: 0.25, lowpass: 950 });
      inst.mix('epiano:lead', { level: 1.3, pan: -0.12, reverb: 0.34, delay: 0.16, tremolo: 0.2, lowpass: 3400 });
      inst.mix('bass', { level: 0.43, lowpass: 1200 });
      inst.mix('pad', { level: 0.42, lowpass: 1700, reverb: 0.5 });
      inst.mix('kick', { level: 0.44 });
      inst.mix('snare', { level: 0.72, pan: 0.06, lowpass: 9000, reverb: 0.16 });
      inst.mix('rim', { level: 0.85, pan: 0.18, reverb: 0.18 });
      inst.mix('hat', { level: 3.2, pan: 0.36, highpass: 5000, lowpass: 10000 });
      inst.mix('shaker', { level: 1.3, pan: -0.38 });
      ...
      inst.vinyl(t0, player.endTime - 0.5, 0.5);
    };
    ...
    return song(env, {
      ...
      bar(b: BarInfo) {
        ...
        for (const h of p.comp) {
          const t = at(h.beat, 6);
          h.notes.forEach((m, j) => inst.epiano(t + h.roll * j, m, sec(h.dur), h.vel * dyn * (j === h.notes.length - 1 ? 1.04 : 1), { bright: 0.2 }));
        }
        ...
        for (const d of p.drums) {
          ...
          switch (d.k) {
            ...
            case 'hat': inst.hat(at(beat, 5), humVel(rng, v, 0.03), { decay: 0.035 }); break;
            case 'ohat': inst.hat(at(beat, 5), v, { open: 0.16 }); break;
            case 'shaker': inst.shaker(at(beat, 6), v, { len: 0.08 }); break;
